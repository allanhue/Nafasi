...
const RENT_PAYMENTS: RentPayment[] = [
...
const UPCOMING_BOOKINGS: Booking[] = [
  { space: 'Studio A',   guest: 'Kelvin Mutua',    date: 'Apr 12', time: '9am–1pm',   amount: 'KES 4,000',  status: 'confirmed' },
  { space: 'Event Hall', guest: 'Sarah Nyambura',  date: 'Apr 14', time: '2pm–8pm',   amount: 'KES 18,000', status: 'confirmed' },
  { space: 'Studio A',   guest: 'Mike Oloo',       date: 'Apr 15', time: '10am–2pm',  amount: 'KES 4,000',  status: 'pending' },
];

const OCCUPANCY_PROPERTIES = [
...
const EARNINGS_CHART = [22, 18, 35, 28, 42, 38, 42];
const EARNINGS_MONTHS = ['O', 'N', 'D', 'J', 'F', 'M', 'A'];
...
const PAYMENT_STATUS_CLASS: Record<RentPayment['status'], string> = {
  paid:    'badge-success',
...
function SkeletonCard() {
  return (
    <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: 6, animation: 'pulse 2s infinite' }}>
      <div style={{ height: 16, background: 'var(--bg-muted)', borderRadius: 4, width: '60%' }} />
      <div style={{ height: 24, background: 'var(--bg-muted)', borderRadius: 4, width: '40%' }} />
...
function StatCardItem({ stat, miniData, isLoading }: { stat: StatCard; miniData: number[]; isLoading?: boolean }) {
  if (isLoading) return <SkeletonCard />;

  const deltaColor =
    stat.deltaDir === 'up'   ? 'var(--status-success)' :
    stat.deltaDir === 'down' ? 'var(--status-danger)'  :
    'var(--text-muted)';
      ...
      <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 8 }}>
        <div>
          <p style={{
            ...
            color: stat.color || 'var(--text-primary)', lineHeight: 1, marginBottom: 4,
          }}>
            {stat.value}
          </p>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            {stat.delta && (
              <span style={{ display: 'flex', alignItems: 'center', gap: 2, fontSize: 11, fontWeight: 500, color: deltaColor }}>
                {stat.deltaDir === 'up'   ? <TrendingUp size={12} />   : null}
                {stat.deltaDir === 'down' ? <TrendingDown size={12} /> : null}
                {stat.delta}
              </span>
            )}
...
function ProgressBar({ value, color, height = 6 }: { value: number; color: string; height?: number }) {
  return (
    <div style={{ height, borderRadius: 999, background: 'var(--bg-muted)', overflow: 'hidden' }}>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginTop: 24 }}>
      ...
      <div className="card">
        ...
        {RENT_PAYMENTS.length > 0 ? (
              ...
              <tbody>
                {RENT_PAYMENTS.map(p => (
                  <tr key={p.unit}>
                    ...
                    <td>
                      <span className={`badge ${PAYMENT_STATUS_CLASS[p.status]}`}>{p.status}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <p style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '20px 0' }}>No payments yet</p>
        )}
      </div>
      ...
      <div className="card">
        ...
        <div style={{ marginBottom: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
            <span style={{ fontSize: 13, color: 'var(--text-secondary)' }}>Overall</span>
            <span style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 14 }}>21 / 24</span>
          </div>
          <ProgressBar value={87.5} color="var(--rental-color)" height={10} />
        </div>

        {OCCUPANCY_PROPERTIES.map(prop => (
          <div key={prop.property} style={{ marginBottom: 14 }}>
            ...
            <ProgressBar
              value={(prop.occupied / prop.total) * 100}
              color={prop.occupied === prop.total ? 'var(--brand-accent)' : 'var(--rental-color)'}
            />
          </div>
        ))}
      </div>
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginTop: 24 }}>
      ...
      <div className="card">
        ...
        {LOW_STOCK.length > 0 ? (
              ...
              <tbody>
                {LOW_STOCK.map(item => (
                  <tr key={item.sku}>
                    ...
                    <td>
                      <span style={{ fontFamily: 'var(--font-display)', fontWeight: 700, color: 'var(--status-danger)' }}>
                        {item.qty}
                      </span>
                    </td>
          ...
          <p style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '20px 0' }}>All items in stock</p>
        )}
      </div>
      ...
      <div className="card">
        ...
        {WAREHOUSES.map(w => (
          <div key={w.name} className="card" style={{ background: 'var(--bg-subtle)', marginBottom: 12, border: 'none', padding: '14px 16px' }}>
            ...
            <ProgressBar value={w.fill} color="var(--inventory-color)" />
...
function SpacesPanel() {
  const maxEarning = Math.max(...EARNINGS_CHART);
    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginTop: 24 }}>
      ...
      <div className="card">
        ...
        {UPCOMING_BOOKINGS.length > 0 ? (
            ...
            {UPCOMING_BOOKINGS.map((b, i) => (
              <div key={i} style={{
                ...
                <div style={{
                  ...
                  background: 'var(--spaces-bg)', border: '1px solid var(--spaces-border)',
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <p style={{ fontSize: 11, color: 'var(--text-muted)' }}>{b.guest} · {b.date}, {b.time}</p>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <p style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: 13, color: 'var(--spaces-color)' }}>
                    {b.amount}
                  </p>
                  <span className={`badge ${b.status === 'confirmed' ? 'badge-success' : 'badge-warning'}`} style={{ fontSize: 10 }}>
                    {b.status}
                  </span>
                </div>
              </div>
          ...
          <p style={{ textAlign: 'center', color: 'var(--text-muted)', padding: '20px 0' }}>No upcoming bookings</p>
        )}
      </div>
      ...
      <div className="card">
        ...
        <div style={{ display: 'flex', alignItems: 'flex-end', gap: 4, height: 80 }}>
          {EARNINGS_CHART.map((v, i) => (
            <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
              <div style={{
                width: '100%',
                height: `${(v / maxEarning) * 68}px`,
                minHeight: 4,
                borderRadius: '3px 3px 0 0',
                background: i === EARNINGS_CHART.length - 1 ? 'var(--spaces-color)' : 'var(--spaces-bg)',
                border: '1px solid var(--spaces-border)',
                transition: 'height 0.4s ease',
              }} />
              <span style={{ fontSize: 10, color: 'var(--text-muted)' }}>{EARNINGS_MONTHS[i]}</span>
            </div>
          ))}
        </div>
      </div>
...
export default function DashboardPage() {
  ...
  const MINI_DATA_BASE = [40, 55, 48, 62, 58, 70, 75];
  ...
  return (
    <div>
        ...
        {isLoading
          ...
          : TAB_META[activeTab].stats.map((stat, i) => (
              <StatCardItem
                key={stat.label}
                stat={stat}
                miniData={MINI_DATA_BASE.slice(i)}
                isLoading={isLoading}
              />
            ))
        }
      ...
      {/* Tab panels */}
      {!isLoading && activeTab === 'rental'    && <RentalPanel />}
      {!isLoading && activeTab === 'inventory' && <InventoryPanel />}
      {!isLoading && activeTab === 'spaces'    && <SpacesPanel />}
